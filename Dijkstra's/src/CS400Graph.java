...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            List<NodeType> temp = new LinkedList<>();
            for (NodeType target  : copyPath.dataSequence) {
                temp.add(target);
            }
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        // case if start or/and end are not found
        ...
        while (queue.size() != 0 && visited.size() < vertices.size()) {
            // remove first path from queue and check if its leaving edges are in priority queue,
            // if not, add it to the priority queue
            Path temp = queue.remove();
            //System.out.println(queue.toString());
            //System.out.println(visited.toString());
            for (int i = 0; i < temp.end.edgesLeaving.size(); i++) {
                if (!visited.contains(temp.end.edgesLeaving.get(i).target)) {
                    Path temp2 = new Path(shortestPath, temp.end.edgesLeaving.get(i));
                    if (shortestPath.compareTo(temp2) == -1) {
                        shortestPath = temp2;
                        visited.add(temp.end.edgesLeaving.get(i).target.data);
                    }
                    queue.add(temp2);
                }
            }
        ...
        // returns if reached end
        if (shortestPath.end.data == end) {
            return shortestPath;
        }
        // throw exception if no path found
        else {
